...
<table class="table form-table mdocs-settings-table">
	...
	<tr>
		...
		<td>
			...
			?>
			<input type="checkbox" name="mdocs-allow-upload[<?php echo $index; ?>]" value="1"  <?php checked($checked , 1) ?> /> <span><?php echo ucfirst($role['name']); ?></span><br>
			<?php
				} 
			} 
			?>
		...
		<td>
			...
			foreach($wp_roles as $role => $value) {
				if($role != 'administrator') {
					?>
					<input type="checkbox" name="mdocs-view-private[<?php echo $role; ?>]" value="1"  <?php checked($mdocs_roles[$role] , 1) ?> /> <span><?php echo ucfirst($value['name']); ?></span><br>
					<?php
				}
			}
			?>
