...
function mdocs_errors($error, $type='updated') {
	if($type == 'error') $error = '<em>'.__('Memphis Error','memphis-documents-library').': </em>'.$error;
	else $error = '<em>'.__('Memphis Info','memphis-documents-library').': </em>'.$error;
	?>
	...
	$html = '
			<tr class="mdocs-parent-cat" >
				<td colspan="'.mdocs_get_num_cols().'" id="title" class="mdocs-tooltip">
					'.$tabs['parent-tab'].'<a href="'.$permalink.$parent['slug'].'&mdocs-att='.$att.'" title="'.$permalink.$parent['slug'].'"><i class="fa fa-folder-o" aria-hidden="true"></i> '.$parent['name'].'</a>
				</td>
			</tr>';
	echo $html;
	...
	$current_cat_string = '
		<tr class="mdocs-current-cat" >
			<td colspan="'.$num_cols.'" id="title" class="mdocs-tooltip">
				'.$tabs['current-tab'].'<i class="fa fa fa-folder-open-o" aria-hidden="true"></i> '.$current_cat['name'].'
			</td>
		</tr>';
	if(isset($current_cat['children']) && count($current_cat['children']) > 0) {
		foreach($current_cat['children'] as $index => $child) {
			...
			<tr class="mdocs-sub-cats" >
				<td colspan="'.$num_cols.'" id="title" class="mdocs-tooltip">
					'.$tabs['child-tab'].'<a href="'.$permalink.$child['slug'].'&mdocs-att='.$att.'" title="'.$child['name'].'"><i class="fa fa-folder-o" aria-hidden="true"></i> '.$child['name'].'</a>
				</td>
			</tr>';
